...
This chapter shows how the design from Chapter~3 appears in the
...
frontend runtimes. The backend uses Python~3 with FastAPI~0.133.1,
SQLAlchemy~2.0.48, Alembic~1.7.7, ChromaDB~1.5.5, OpenAI~2.26.0,
Google Cloud Vision~3.11.0, Pillow~12.0.0, Pydantic~2.12.5,
Pydantic Settings~2.12.5, and Uvicorn~0.41.0. The frontend uses
React~19.2.0, React Router DOM~7.13.1, and Vite~7.3.1. These exact
versions are defined in \texttt{backend/requirements.txt} and
\texttt{frontend/package.json}, which makes the implementation
reproducible instead of describing only a generic stack. The backend
technology choices follow the official Python and FastAPI
documentation \cite{python2026setup,fastapi2026docs}. Persistence and
schema management follow the SQLAlchemy and Alembic documentation
\cite{sqlalchemy2026docs,alembic2026docs}. Retrieval and AI-service
integration follow the ChromaDB, OpenAI, and Google Cloud Vision
documentation \cite{chromadb2026docs,openai2026platform,google2026vision}.
Supporting backend components such as image processing, validation, and
application serving follow the Pillow, Pydantic, Pydantic Settings, and
Uvicorn documentation
\cite{pillow2026docs,pydantic2026docs,pydanticsettings2026docs,uvicorn2026docs}.
The frontend stack follows the React, React Router, and Vite
documentation \cite{react2026docs,reactrouter2026docs,vite2026docs}.

The development environment was intentionally lightweight and local.
Source code was edited primarily in Visual Studio Code, while Git was
used for version control across both frontend and backend changes. The
backend runtime was managed through a Python virtual environment and the
dependency manifest in \texttt{backend/requirements.txt}. The frontend
runtime was managed through Node.js and the dependency manifests
\texttt{frontend/package.json} and \texttt{frontend/package-lock.json}.
Database schema evolution was handled through Alembic migration files,
and LaTeX source for the thesis itself was maintained in the same
repository so that code and documentation could evolve together.
...
This file system mirrors the layered design from Chapter~3. Route
...
detail later in Section~4.5 because it spans relational schema design,
...
routers. This directly realizes the component diagram from Figure~3.2.
...
\texttt{backend/security.py} contains password hashing and JWT helpers,
...
restriction logic discussed in Chapter~3. It is also one of the points
...
collection. The retrieval call is implemented in
\texttt{backend/services/rag\_service.py}:
...
The prompt sent to the LLM is not generic. It is assembled from the
...
of the workflow diagrams and design rationale from Chapter~3.
...
\texttt{backend/services/ocr\_image\_processing.py}:
...
The second OCR stage turns raw text into structured JSON. The LLM prompt
shows that the expected schema is explicit and validated later through
Pydantic:
...
HTTP communication is centralized in \texttt{frontend/src/lib/api.js}.
This module provides a single base URL resolver and a reusable
...
\texttt{frontend/src/context/AuthContext.jsx}:
...
The relational design from Chapter~3 is not only diagrammed; it is
...
The relational model from Chapter~3 is implemented in
\texttt{backend/models.py}. The \texttt{GeneratedRecipe} entity is the
...
Schema evolution is managed through Alembic. The migration
...
index strategy described in Chapter~3 is connected to an actual access
...
\texttt{backend/data/recipes\_subset.json}. This indexing script shows
that the retrieval layer is not abstract or
purely conceptual. The corpus preparation and embedding workflow are
implemented as executable code that can rebuild the persistent Chroma
store used during generation.
...
The default relational database is SQLite, as configured in
...
keeps the schema compact and matches the JSON-shaped API contracts well,
...
main intelligent features. Recipe generation, embedding creation, OCR
text structuring, and recipe-image generation all require upstream API
access and therefore do not currently support a fully offline execution
mode. This is acceptable for the current thesis-scale deployment model,
but it defines a clear operational boundary for the implementation.
...
Google Vision-based implementation is currently available in production
form. Likewise, recipe image generation is represented through a
provider interface, but only the OpenAI-backed path is implemented.
...
Chapter~3 presented the architecture and design rationale of the
project; this chapter showed their realization in code. The FastAPI
bootstrap, route modules, service classes, and React context/page
structure demonstrate that the UML and architectural diagrams reflect
the actual repository. The persistence layer, Alembic revisions, and
Chroma indexing script likewise show that the database and retrieval
designs were implemented as executable program components rather than
remaining design-only artifacts. This concrete implementation basis is
used in the next chapter to evaluate functional behavior, robustness,
and output quality.
